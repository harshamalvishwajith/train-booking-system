...
import { Clock, MapPin, Train as TrainIcon, Users, CreditCard } from "lucide-react";

async function TrainResults({ origin, destination, date }: { origin: string, destination: string, date: string }) {
  let trains: Train[] = [];
  try {
    const response = await fetchTrains(origin, destination, date);
    trains = Array.isArray(response) ? response : (response?.data || response?.schedules || response?.trains || []);
  } catch (error) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center text-red-500 bg-red-500/10 rounded-xl border border-red-500/20">
        ...
        <p>Could not connect to the Train Management Service. Please try again later.</p>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="p-6 flex-1 flex flex-col justify-between">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-border/40 gap-4">
                ...
                <Link href={`/book/${train.scheduleId}`} className="w-full sm:w-auto">
...
// NextJS 15 Page with Async Search Params
export default async function SearchPage(props: {
  ...
  try {
    if (dateStr) displayDate = format(new Date(dateStr), "MMMM do, yyyy");
  } catch(e) {}
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <Suspense fallback={
        ...
        <TrainResults origin={origin} destination={destination} date={dateStr} />
      </Suspense>
    </div>
  );
}
